...
if(__name__ == '__main__'):
    ...
    for epoch in range(opt.max_epoch):
        ...
        for (i, batch) in enumerate(train_loader):
            ...
            writer.add_scalar('data/scalar_loss_layer-2-768', loss.detach().cpu().numpy(), iteration)
            ...
            if (iteration % 100 == 0 or iteration == 1):
                with torch.no_grad():
                    ...
                    for batch in tst_loader:
                        ...
                        predict_txt = MyDataset.key2word(outputs.argmax(-1))
                        truth_txt = MyDataset.key2word(decoder_tensor)
                ...
                writer.add_scalar('data/scalar_wer_layer-2-768', wer, n)
                #cer = MyDataset.cer(predict_txt_total, truth_txt_total)                
                #print('cer:{}, wer:{}'.format(cer, wer))       
                print('wer:{}'.format(wer))   
                print(''.join(101*'-'))
                savename = os.path.join(opt.save_dir, 'iteration_{}_epoch_{}_wer_{}_layer-2-768.pt'.format(iteration, epoch, wer))
                savepath = os.path.split(savename)[0]
                if(not os.path.exists(savepath)): os.makedirs(savepath)
                if wer <= 0.12: torch.save(model.state_dict(), savename)
                n += 1
